...
type Channel struct {
	ID        string      `json:"id" gorm:"primaryKey"`
	Name      string      `json:"name" gorm:"not null;uniqueIndex:idx_channel_server"`
	ServerID  string      `json:"serverId" gorm:"not null;uniqueIndex:idx_channel_server"`
...
type Member struct {
	ID       string    `json:"id" gorm:"primaryKey"`
	UserID   string    `json:"userId" gorm:"not null;uniqueIndex:idx_user_server"`
	ServerID string    `json:"serverId" gorm:"not null;uniqueIndex:idx_user_server"`
...
type Message struct {
	ID         string    `json:"id" gorm:"primaryKey"`
	ChannelID  string    `json:"channelId" gorm:"not null;index:idx_channel_time"`
...
// MemberDeviceKey stores a user's P-256 ECDH public key, scoped per server.
// Supports multiple devices per user — one row per (user_id, device_id, server_id).
type MemberDeviceKey struct {
	ID        string    `json:"id" gorm:"primaryKey"`
	UserID    string    `json:"userId" gorm:"not null;index:idx_device_user_server"`
	DeviceID  string    `json:"deviceId" gorm:"not null;index:idx_device_user_server"`
	ServerID  string    `json:"serverId" gorm:"not null;index:idx_device_user_server"`
...
type ChannelKeyBundle struct {
	ID                  string    `json:"id" gorm:"primaryKey"`
	ChannelID           string    `json:"channelId" gorm:"not null;index:idx_bundle_channel"`
