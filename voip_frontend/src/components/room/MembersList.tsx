...
export default function MemberList() {
    const { connectedPeers, isVoiceActive, muted, setPeerVolume, peerVolumes } = useVoIPContext();
    const [contextMenu, setContextMenu] = useState<ContextMenu | null>(null);
    const menuRef = useRef<HTMLDivElement>(null);

    const handleContextMenu = useCallback((e: React.MouseEvent, peerId: string, label: string) => {
        e.preventDefault();
        ...
        <>
            <div className="space-y-1">
                ...
                {connectedPeers.map((peer) => (
                    <div
                        key={peer.peerId}
                        onContextMenu={(e) => handleContextMenu(e, peer.peerId, peer.alias || peer.peerId.slice(0, 12))}
                ...
                >
                    ...
                    <div className="px-2 py-1 space-y-1">
                        <div className="flex justify-between text-muted-foreground">
                            <span>Volume</span>
                            <span>{peerVolumes[contextMenu.peerId] ?? 100}%</span>
                        </div>
                        <input
                            type="range"
                            min={0}
                            max={200}
                            step={5}
                            value={peerVolumes[contextMenu.peerId] ?? 100}
                            onChange={(e) => setPeerVolume(contextMenu.peerId, Number(e.target.value))}
                            className="w-full"
                        />
