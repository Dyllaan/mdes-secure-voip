...
test.describe('Auth - Login page', () => {
  ...
  test('successful login redirects off the login page', async ({ page }) => {
    ...
    // After login there are no IDB keys, so KeysRequired redirects to /keys
...
test.describe('Auth - Register page', () => {
  ...
  test('password shorter than 8 chars is rejected by browser validation', async ({ page }) => {
    // The password input has minLength={8} as an HTML attribute, so the browser
    // prevents form submission natively before any React code runs.
    await page.goto('/register');
    await page.getByTestId('username-input').fill('newuser');
    ...
    // Form stays on the register page — native validation blocked submission
    await expect(page.getByTestId('register-submit')).toBeVisible();
  ...
  test('successful registration shows MFA setup dialog', async ({ page }) => {
    ...
    // MfaSetupDialog calls /mfa/status
    await mockAuthRoutes(page);
    ...
    // MfaSetupDialog should appear (contains a "Skip" or similar button)
